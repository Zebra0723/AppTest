...
function pickProvider() {
  if (process.env.GROQ_API_KEY) return 'groq';   // free, default
  if (geminiKey()) return 'gemini';               // free
  if (process.env.OPENAI_API_KEY) return 'openai'; // paid
  return null;
}
...
export async function aiReview(audit, features) {
  ...
  if (!provider) {
    return {
      available: false,
      reason: 'No AI key set. Add a FREE Groq key as GROQ_API_KEY ' +
        '(get one at https://console.groq.com/keys — no billing needed).',
    };
  }
