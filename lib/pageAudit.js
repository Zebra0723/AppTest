...
export function featureProbe(feature) {
  ...
  // Only consider *visible* elements — hidden (display:none) buttons/inputs must
  // not count as a present feature (e.g. an app hidden behind a login gate).
  const visible = (el) => el.getClientRects().length > 0;
  const attrs = Array.from(document.querySelectorAll('[aria-label],[placeholder],[alt],[title],button,a,input'))
    .filter(visible)
    .map((e) => [e.getAttribute('aria-label'), e.getAttribute('placeholder'), e.getAttribute('alt'),
      e.getAttribute('title'), e.value, e.innerText].filter(Boolean).join(' ')).join(' ');
