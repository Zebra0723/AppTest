...
export async function runAudit({ url, features = [], viewports = DEFAULT_VIEWPORTS, waitExtraMs = 3500, auth = null }) {
  const result = {
  ...
  try {
    ...
    // Let the page actually finish loading before we judge it, so the audit and
    // the AI don't react to a half-rendered page:
    //  1. scroll through it to trigger lazy-loaded images/sections,
    //  2. wait for loading spinners/skeletons to disappear (bounded),
    //  3. wait for the network to go quiet again,
    //  4. a final settle pause.
    await autoScroll(page);
    await waitForLoadersToClear(page, 5000);
    ...
    for (let i = 0; i < viewports.length; i++) {
      ...
      a.name = vp.name;
      try {
        const buf = await page.screenshot({ fullPage: i === 0, type: 'png' });
    ...
    if (stage === 'launch') result.error = `Could not start the browser. ${msg}`;
    else if (!result.reachable) result.error = `Navigation failed: ${msg}`;
    else result.error = `${e && e.name ? e.name : 'Error'}: ${msg}`;
  } finally {
  ...
  try {
    await page.evaluate(async () => {
      await new Promise((resolve) => {
        ...
        setTimeout(() => { clearInterval(timer); resolve(); }, 4000); // hard cap
      });
    });
...
// Wait (bounded) for visible loading spinners / skeletons to disappear, so we
// don't screenshot or grade a page that's still fetching its data.
async function waitForLoadersToClear(page, timeout) {
  const start = Date.now();
  while (Date.now() - start < timeout) {
    let visible = 0;
    try {
      visible = await page.evaluate(() => {
        const sels = ['[class*="spinner" i]', '[class*="loader" i]', '[class*="loading" i]',
        ...
        for (const s of sels) {
          ...
          for (const n of nodes) {
            ...
            if (r.width > 0 && r.height > 0 && cs.visibility !== 'hidden' &&
                cs.display !== 'none' && parseFloat(cs.opacity || '1') > 0.05) c++;
          }
        }
        return c;
      });
